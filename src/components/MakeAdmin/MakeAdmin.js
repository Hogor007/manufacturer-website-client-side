...
const MakeAdmin = () => {
  ...
  const handleMakeAdmin = (email) => {
    console.log(email);
    setReload(true);
    fetch(`https://enigmatic-beyond-17898.herokuapp.com/user/admin/${email}`, {
      method: "PUT",
      headers: {
  ...
  const handleRemoveAdmin = (email) => {
    console.log("Putin", email);
    if (email === authUser?.email) {
      toast.error("You can't remove yourself from admin");
      return;
    }
    setReload(true);
    fetch(`https://enigmatic-beyond-17898.herokuapp.com/remove/admin/${email}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        authorization: `Bearer ${localStorage.getItem("accessToken")}`,
  ...
  return (
    <div>
      <h3 className="text-center text-primary mb-4">Make Admin</h3>
      {isLoading ? (
        <Loading></Loading>
      ) : (
        <div>
